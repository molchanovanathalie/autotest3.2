...
@pytest.fixture()
def make_badarx():
    checkout_positive("cd {}; 7z a {}/arch.7z".format(data["folder_in"], data["folder_arch"]), "Everything is Ok")
    checkout_positive("truncate -s l {}/arch.7z".format(data["folder_arch"]), "Everything is Ok")
    yield "arch"
    checkout_positive("rm -f {}/arch.7z".format(data["folder_arch"]), "")
